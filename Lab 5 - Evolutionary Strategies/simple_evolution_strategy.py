...
class SimpleEvolutionStrategy:
    ...
    def tell(self, fitnesses):
        ...
        best_samples = (self.samples[(np.array(fitnesses).argsort())[::1]])[0:self.mu]

        self.C = 0
        for i in range(self.mu):
            self.C += (np.matrix(best_samples[i] - self.m)).transpose() * np.matrix(best_samples[i] - self.m)
        self.C /= self.mu

        self.m = (np.sum(best_samples, 0)) / self.mu
