...
class LineFollower:
    ...
    """
    def __init__(self, pose, rl_algorithm, linear_speed, robot_params, sensor_params):
        """
    ...
    def control(self):
        ...
        if detected:
            ...
            error_normalized = (error + self.line_sensor.array_width / 2.0) / self.line_sensor.array_width
        ...
        if self.previous_sar is not None:  # if (S, A, R) is None, then we are at the first iteration
            ...
            self.rl_algorithm.learn(previous_state, previous_action, previous_reward, state, action)
        # Making the action continuous
        angular_speed = (action / num_actions) * 2.0 * self.max_angular_speed - self.max_angular_speed
