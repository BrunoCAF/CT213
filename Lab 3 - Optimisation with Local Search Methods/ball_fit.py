...
def fit_hill_climbing():
    ...
    def neighbors(theta):
        ...
        for k in range(num_neighbors):
            neighbors_list.append(theta + delta * np.array([cos(k*angle),sin(k*angle)]))
...
def fit_simulated_annealing():
    ...
    def random_neighbor(theta):
        ...
        neighbor = theta + delta*np.array([cos(angle), sin(angle)])
        return neighbor
...
def plot_optimization(history, color=''):
    """
    ...
    :param color: color of the plotted curve.
    ...
    for h in history:
        hx.append(h[0])
        hy.append(h[1])
    handle, = plt.plot(hx, hy, '.-%s' % color, markersize=5)
    plt.xlabel('v_0 (m/s)')
    plt.ylabel('f (m/s^2)')
...
plt.figure()
plot_optimization(history_gd, 'y')
...
plt.figure()
plot_optimization(history_hc, 'b')
...
plt.figure()
plot_optimization(history_sa, 'r')
...
plt.figure()
handle_gd = plot_optimization(history_gd, 'y')
handle_hc = plot_optimization(history_hc, 'b')
handle_sa = plot_optimization(history_sa, 'r')
...
plt.plot(t, v_ls, 'tab:green')
plt.plot(t, v_gd, 'tab:orange')
plt.plot(t, v_hc, 'tab:blue')
plt.plot(t, v_sa, 'tab:red')
plt.xlabel('Time (s)')
